...
# Attempt to create Gradio Interface with theme if supported; fallback to no-theme
def make_demo():
    common_kwargs = dict(
        fn=gradio_interface,
        inputs=[
    ...
    # First try with theme (some Gradio versions support it)
    try:
        demo_with_theme = gr.Interface(**common_kwargs, theme=gr.themes.Soft())
        return demo_with_theme
    except TypeError:
        # Fallback: create without theme argument
        return gr.Interface(**common_kwargs)


demo = make_demo()

# Mount Gradio to FastAPI at /ui
